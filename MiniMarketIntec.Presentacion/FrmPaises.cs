...
namespace MiniMarketIntec.Presentacion
{
    public partial class FrmPaises : Form
    {
        ...
        private void Formato()
        {
            ...
            dgvListado.Columns[0].HeaderText = "Código Pais";
            dgvListado.Columns[1].Width = 250;
            dgvListado.Columns[1].HeaderText = "Nombre Pais";
            dgvListado.Columns[2].HeaderText = "Estado";
        }

        //Listar las categorias en el Data Grid View
        private void ListarPaises(string valor)
        {
            try
            {
                //llenamos el dgv
                dgvListado.DataSource = NPais.ListarPaises(valor);
            }
            catch (Exception ex)
            {
        ...
        {
            if (!string.IsNullOrEmpty(Convert.ToString(dgvListado.CurrentRow.Cells["codigo_pais"].Value)))
            {
                txtDescripcion.Text = dgvListado.CurrentRow.Cells["descripcion_pais"].Value.ToString();
                txtId.Text = dgvListado.CurrentRow.Cells["codigo_pais"].Value.ToString();
            }
            else
            {
                MensajeError("Debe seleccionar un Pais");
            }
        }
        ...
        private void btnGuardar_Click_1(object sender, EventArgs e)
        {
            ...
            {
                errorProvider.SetError(txtDescripcion, "Ingrese un Nombre para el Pais");
            }
            else
            {
                ...
                {
                    MensajeError("El Pais ya Existe");
                }
                else
                {
                    //limpiamos cualquier mensaje de error
                    errorProvider.Clear();
                    Respuesta = NPais.RegistrarPais(opcionGuardar, 0, txtDescripcion.Text.Trim());
                    if (Respuesta == "OK")
                    {
                        //La categoria se registró satisfactoriamente
                        MensajeOK("El Pais se Registró Adecuadamente");
                        opcionGuardar = 0;
                        EstadoBotones(true);
        ...
        {
            this.ListarPaises(txtBuscar.Text.Trim());
        }
        ...
        {
            opcionGuardar = 2; //deseamos actualizar del pais
            string Respuesta = "";
            ErrorProvider errorProvider = new ErrorProvider();
            ...
            else
            {
                ...
                Respuesta = NPais.RegistrarPais(opcionGuardar, int.Parse(txtId.Text), txtDescripcion.Text.Trim());
                if (Respuesta == "OK")
                {
                    MensajeOK("El Pais se actualizó correctamente");
                    opcionGuardar = 0;
                    EstadoBotones(true);
                    EstadoBotonesProcesos(false);
                    txtDescripcion.Text = "";
                    txtId.Text = "";
                    txtDescripcion.Enabled = false;
                    this.ListarPaises("%"); //refrescar el datagridview
                    tabPrincipal.SelectedIndex = 0;
                }
                else
                {
        ...
        private void btnEliminar_Click_1(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(Convert.ToString(dgvListado.CurrentRow.Cells["codigo_pais"].Value)))
            {
                if (MessageBox.Show("¿Seguro que desea eliminar el Pais?", "Eliminar Pais", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    //desactivamos el rubro
                    //obtener el id de la fila o rubro seleccionado
                    int codigoPais = Convert.ToInt32(dgvListado.CurrentRow.Cells["codigo_pais"].Value);
                    string Respuesta = NPais.Desactivar(codigoPais);
                    if (Respuesta == "OK")
                    {
                        MensajeOK("El Pais se eliminó correctamente");
                        this.ListarPaises("%");
                    }
